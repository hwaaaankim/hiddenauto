document.addEventListener("DOMContentLoaded", function() {
    ...
    function setDeleteButtonsBusy(isBusy) {
        ...
        [deleteTasksBtn, deleteOrdersBtn].forEach(btn => {
            if (!btn) {
                return;
            }
    ...
    function readMemberSource(root) {
        const base = root || document;
        return Array.from(base.querySelectorAll(".admin-task-list-second-member-source [data-member-id], #admin-task-list-second-member-source [data-member-id]"))
            .map(item => ({
    ...
    function syncRequesterSelect(companySelect) {
        ...
        if (!form || !requesterSelect) {
            return;
        }

        const memberOptions = readMemberSource(form.closest(".admin-task-list-second-detail-panel") || form);
        const selectedCompanyId = String(companySelect.value || companySelect.dataset.selectedCompanyId || "");
        const selectedMemberId = String(requesterSelect.dataset.selectedMemberId || requesterSelect.value || "");
        ...
        memberOptions
            .filter(member => member.companyId === selectedCompanyId)
            .forEach(member => {
    ...
    function initializeCompanyMemberSelects(root) {
        const base = root || document;

        base.querySelectorAll(".admin-task-list-second-company-select").forEach(companySelect => {
            syncRequesterSelect(companySelect);

            if (companySelect.dataset.companyMemberInitialized === "true") {
                return;
            }

            companySelect.dataset.companyMemberInitialized = "true";
        ...
        base.querySelectorAll(".admin-task-list-second-requester-select").forEach(requesterSelect => {
            if (requesterSelect.dataset.requesterInitialized === "true") {
                return;
            }

            requesterSelect.dataset.requesterInitialized = "true";
    ...
    function findDetailRow(orderId) {
    ...
    function openExistingDetailRow(orderId, forceOpen) {
        ...
        setWideButtonState(button, nextOpen, false);
        return true;
    }
    ...
    async function loadDetailRow(orderId) {
        const mainRow = document.querySelector(`.admin-task-list-second-main-row[data-order-id="${orderId}"]`);
        const button = document.querySelector(`.admin-task-list-second-wide-toggle-btn[data-order-id="${orderId}"]`);
        ...
        if (detailLoadingOrderIds.has(String(orderId))) {
            return;
        }

        detailLoadingOrderIds.add(String(orderId));
        setWideButtonState(button, false, true);

        try {
            const url = new URL(`/management/nonStandardTaskList/order-detail-fragment/${orderId}`, window.location.origin);
            ...
            const detailRow = template.content.querySelector(".admin-task-list-second-detail-row");

            if (!detailRow) {
                throw new Error("주문 상세 화면 응답 형식이 올바르지 않습니다.");
            }

            mainRow.insertAdjacentElement("afterend", detailRow);

            initializeCompanyMemberSelects(detailRow);
            initializeImageSection(detailRow);

            if (window.AdminTaskListSecondOrderEditExtension?.initForm) {
                const form = detailRow.querySelector(".admin-task-list-second-update-form");
                window.AdminTaskListSecondOrderEditExtension.initForm(form);
            }

            setWideButtonState(button, true, false);
        } catch (error) {
            alert(error.message || "주문 상세 정보를 불러오는 중 오류가 발생했습니다.");
            setWideButtonState(button, false, false);
        } finally {
            detailLoadingOrderIds.delete(String(orderId));
        }
    }

    async function toggleDetailRow(orderId, forceOpen) {
        if (!orderId) {
            return;
        }

        const existingRow = findDetailRow(orderId);

        if (existingRow) {
            openExistingDetailRow(orderId, forceOpen);
            return;
        }
    ...
    function initializeImageSection(root) {
        const base = root || document;
        base.querySelectorAll(".admin-task-list-second-image-section").forEach(refreshNewImageEmptyState);
    }
    ...
    function initializeBaseEvents() {
        ...
        document.addEventListener("submit", function(event) {
            if (event.target.matches(".admin-task-list-second-update-form")) {
                showPageLoading("처리 중입니다. 잠시만 기다려 주세요.");
            }
        });

        document.querySelectorAll("form:not(.admin-task-list-second-update-form)").forEach(form => {
    ...
    initializeCompanyMemberSelects(document);
    initializeImageSection(document);
