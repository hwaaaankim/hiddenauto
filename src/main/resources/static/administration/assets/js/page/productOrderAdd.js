(function() {
    ...
    function cacheElements() {
        ...
        els.summaryModal = new bootstrap.Modal(els.summaryModalEl);
    }
    ...
    function renderCompanyDropdown() {
        ...
        els.companyDropdown.innerHTML = state.companyResults.map((item, index) => `
            <button type="button"
                ...
                <span class="product-admin-add-autocomplete-title">${escapeHtml(item.companyName)} (${escapeHtml(item.representativeName || '-')})</span>
                <span class="product-admin-add-autocomplete-sub">가입일 ${formatDate(item.joinedAt)} ${item.address ? `ㆍ ${escapeHtml(item.address)}` : ''}</span>
            </button>
        `).join('');
    ...
    function renderDeliveryDropdown() {
        ...
        els.deliveryDropdown.innerHTML = state.deliveryResults.map((item, index) => `
            <button type="button"
                ...
                <span class="product-admin-add-autocomplete-title">${escapeHtml(item.name)} (${escapeHtml(item.username || '-')})</span>
                <span class="product-admin-add-autocomplete-sub">${item.phone ? escapeHtml(item.phone) + ' ㆍ ' : ''}가입일 ${formatDate(item.joinedAt)}</span>
            </button>
        `).join('');
    ...
    function openAddressSearch() {
        ...
        new daum.Postcode({
            oncomplete: function(data) {
                ...
                state.address.zipCode = parsed.zipCode;
                state.address.doName = parsed.doName;
                state.address.siName = parsed.siName;
                state.address.guName = parsed.guName;
                state.address.roadAddress = parsed.roadAddress;

                els.zipCode.value = parsed.zipCode;
                els.doName.value = parsed.doName;
                els.siName.value = parsed.siName;
                els.guName.value = parsed.guName;
                els.roadAddress.value = parsed.roadAddress;
    ...
    function renderAddressSummary() {
        ...
        const region = [state.address.doName, state.address.siName, state.address.guName]
            .filter(item => item && item.trim())
            .join(' / ');

        const fullAddress = [state.address.roadAddress, state.address.detailAddress]
            .filter(item => item && item.trim())
            .join(' ');
    ...
    function validateAddressFields(showMessage = false) {
        ...
        if (showMessage) {
            ...
            els.addressFeedback.textContent = '공통 배송지를 주소검색으로 선택해 주세요.';
        }
    ...
    function renderOrders() {
        ...
        els.orderList.innerHTML = state.orders.map((order, index) => renderOrderCardWrap(order, index)).join('');
        refreshBottomSummary();
        refreshActionButtons();
    }
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-12">
                            <div class="product-admin-add-section-box">
                                ...
                                <div class="product-admin-add-option-help">
                                    예시: <strong>제품명 : 빌트 1도어장</strong> 처럼 입력 후 Enter 를 누르면 등록됩니다.
                                </div>

                                <div class="row g-3">
                                    <div class="col-12 col-lg-9">
                                        <input type="text"
                                            class="form-control product-admin-add-control product-admin-add-option-input"
                                            data-order-id="${order.id}"
                                            placeholder="제목 : 답변 형식으로 입력 후 Enter">
                                    </div>
                        ...
                        <div class="col-12">
                            <div class="product-admin-add-section-box">
                                ...
                                <div class="product-admin-add-upload-box">
    ...
    function renderOptionListHtml(order) {
        ...
        return order.optionPairs.map(pair => `
            <div class="product-admin-add-option-chip">
                <span class="product-admin-add-option-chip-text">${escapeHtml(pair.key)} : ${escapeHtml(pair.value)}</span>
                <button type="button"
    ...
    function renderFileCard(orderId, fileWrapper) {
        const isImage = fileWrapper.file && fileWrapper.file.type && fileWrapper.file.type.startsWith('image/');
        return `
    ...
    function handleOrderListClick(event) {
        ...
        if (action === 'add-option' && order) {
            const input = els.orderList.querySelector(`.product-admin-add-option-input[data-order-id="${orderId}"]`);
            if (input) {
                addOptionFromInput(order, input);
            }
            return;
        }
    ...
    async function handleOrderListChange(event) {
        ...
        if (target.matches('[data-field="standardCategoryId"]')) {
            ...
            order.standardSeriesOptions = order.standardCategoryId ? await loadStandardSeries(order.standardCategoryId) : [];
    ...
    function handleOrderListKeydown(event) {
        ...
        if (target.matches('.product-admin-add-option-input') && event.key === 'Enter') {
            event.preventDefault();
            addOptionFromInput(order, target);
            return;
        }
        ...
        if (target.matches('.product-admin-add-production-input')) {
            ...
            if (event.key === 'ArrowDown') {
                ...
                event.preventDefault();
                order.productionDropdownIndex = Math.min((order.productionDropdownIndex ?? -1) + 1, filtered.length - 1);
                renderProductionDropdown(order);
                return;
            }
            ...
            if (event.key === 'ArrowUp') {
                if (!filtered.length) return;
                event.preventDefault();
                order.productionDropdownIndex = Math.max((order.productionDropdownIndex ?? 0) - 1, 0);
                renderProductionDropdown(order);
                return;
            }
    ...
    function renderProductionDropdown(order) {
        ...
        const items = getFilteredProductionCategories(order.customProductionInput || '');
        order.productionDropdownIndex = items.length ? Math.max(order.productionDropdownIndex ?? 0, 0) : -1;
    ...
    function validateExactProductionSelection(order) {
        ...
        const input = els.orderList.querySelector(`.product-admin-add-production-input[data-order-id="${order.id}"]`);

        if (!value) {
            setProductionFeedback(order.id, '');
            if (input) input.classList.remove('product-admin-add-invalid');
            return true;
        }
        ...
        if (input) input.classList.add('product-admin-add-invalid');
        return false;
    }
    ...
    function getFilteredProductionCategories(keyword) {
        ...
        return state.productionCategories.filter(item => normalizeText(item.name).includes(normalized));
    }

    function addOptionFromInput(order, input) {
        const value = (input.value || '').trim();
        if (!value) {
            return;
        }

        const parsed = parseOptionLine(value);
        if (!parsed) {
            alert('옵션은 "제목 : 답변" 형식으로 입력해 주세요.');
            input.focus();
            return;
        }
    ...
    function updateOptionList(orderId) {
        const order = findOrder(orderId);
        const list = els.orderList.querySelector(`.product-admin-add-option-list[data-order-id="${orderId}"]`);
        if (!order || !list) {
            return;
        }
    ...
    function updateFileList(orderId) {
        const order = findOrder(orderId);
        const list = els.orderList.querySelector(`.product-admin-add-file-preview-list[data-order-id="${orderId}"]`);
        if (!order || !list) {
            return;
        }
    ...
    function assignProductionCategoryByStandardCategory(order) {
        ...
        const standardCategory = state.standardCategories.find(item => String(item.id) === String(order.standardCategoryId));
        if (!standardCategory) {
            return;
        }
        ...
        const matched = state.productionCategories.find(item => normalizeText(item.name) === normalizeText(standardCategory.name));
        if (matched) {
    ...
    function updateOrderCardTotal(orderId) {
        ...
        if (totalCell) {
            totalCell.textContent = formatCurrency(toNumber(order.productCost) * toNumber(order.quantity));
        }
    }
    ...
    function validateOrders(showMessage = false) {
        ...
        for (let i = 0; i < state.orders.length; i++) {
            ...
            if (order.standard === null) {
                if (showMessage) showTopMessage(`주문 ${orderNo}: 규격 / 비규격을 선택해 주세요.`, true);
                return false;
            }

            if (order.standard === true) {
                if (!order.standardCategoryId) {
                    if (showMessage) showTopMessage(`주문 ${orderNo}: 규격 대분류를 선택해 주세요.`, true);
                    return false;
                }
                if (!order.standardSeriesId) {
                    if (showMessage) showTopMessage(`주문 ${orderNo}: 규격 중분류를 선택해 주세요.`, true);
                    return false;
                }
                if (!order.assignedProductionCategoryId) {
                    if (showMessage) showTopMessage(`주문 ${orderNo}: 대분류 기준 생산팀 분류를 찾지 못했고 기본 분류(ID 1)도 없습니다.`, true);
                    return false;
                }
            }
            ...
            if (order.standard === false) {
                if (!order.customProductionCategoryId) {
                    if (showMessage) showTopMessage(`주문 ${orderNo}: 비규격 생산팀 분류를 정확히 선택해 주세요.`, true);
                    return false;
                }
            }

            if (!order.optionPairs.length) {
                if (showMessage) showTopMessage(`주문 ${orderNo}: 최소 1개 이상의 옵션을 입력해 주세요.`, true);
                return false;
            }

            if (`${order.productCost}`.trim() === '' || toNumber(order.productCost) < 0) {
                if (showMessage) showTopMessage(`주문 ${orderNo}: 제품가격을 입력해 주세요.`, true);
                return false;
            }

            if (toNumber(order.quantity) <= 0) {
                if (showMessage) showTopMessage(`주문 ${orderNo}: 수량은 1 이상이어야 합니다.`, true);
                return false;
            }
        }
    ...
    function showTopMessage(message, shouldScroll = false) {
        ...
        if (shouldScroll) {
            els.topMessage.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
    }
    ...
    function renderSummary() {
        const totalPrice = state.orders.reduce((sum, order) => sum + (toNumber(order.productCost) * toNumber(order.quantity)), 0);
            ...
            <div class="product-admin-add-summary-box">
                ...
                <div class="product-admin-add-summary-list">
                    ...
                    <div class="product-admin-add-summary-item">
                        ...
                        <span class="product-admin-add-summary-item-value">${escapeHtml(state.deliveryHandler.name)} (${escapeHtml(state.deliveryHandler.username || '-')})</span>
                    </div>
                    <div class="product-admin-add-summary-item">
    ...
    function getDeliveryAddressSummaryText() {
        const address = [state.address.roadAddress, state.address.detailAddress]
            .filter(item => item && item.trim())
            .join(' ');
        ...
        try {
            const payload = {
                ...
                orders: state.orders.map(order => ({
                    ...
                    standardCategoryId: order.standard ? Number(order.standardCategoryId) : null,
                    standardProductSeriesId: order.standard ? Number(order.standardSeriesId) : null,
    ...
    function getOrderDisplayName(order) {
        const productName = order.optionPairs.find(item => normalizeText(item.key) === normalizeText('제품명'));
        if (productName && productName.value) {
            return productName.value;
        }

        if (order.standard) {
            return getStandardSeriesName(order) || getStandardCategoryName(order.standardCategoryId) || '규격 제품';
        }
    ...
    function getStandardCategoryName(categoryId) {
        const item = state.standardCategories.find(category => String(category.id) === String(categoryId));
        return item ? item.name : '';
    }

    function getStandardSeriesName(order) {
        const item = (order.standardSeriesOptions || []).find(series => String(series.id) === String(order.standardSeriesId));
        return item ? item.name : '';
    }
