...
(function() {
	...
	function setupCustomerMyInfoOrdererUI() {
		...
		var ordererItems = Array.isArray(window.__CUSTOMER_MYINFO_ORDERER_INFOS__)
			? window.__CUSTOMER_MYINFO_ORDERER_INFOS__.map(function(x) {
				return {
					id: x.id || null,
					name: x.name || '',
					phone: x.phone || ''
		...
		function renderOrdererList() {
			...
			if (!ordererItems.length) {
				var empty = document.createElement('div');
				empty.className = 'customer-myInfo-orderer-empty';
			...
			ordererItems.forEach(function(item) {
				...
				row.className = 'customer-myInfo-orderer-item';

				var left = document.createElement('div');
				left.className = 'customer-myInfo-orderer-item-left';

				var name = document.createElement('div');
				name.className = 'customer-myInfo-orderer-name';
				name.textContent = item.name || '-';

				var phone = document.createElement('div');
				phone.className = 'customer-myInfo-orderer-phone';
				...
				deleteBtn.className = 'customer-myInfo-orderer-delete-btn';
		...
		function openModal() {
			...
			if (!modalInstance) {
				alert('모달 라이브러리가 없어 주문자 정보 등록창을 열 수 없습니다. (bootstrap 미로드)');
				return;
			}
			...
			fetch('/customer/api/orderer-infos', {
				...
				.then(function(result) {
					if (!result.ok || !result.body || result.body.success === false) {
						alert((result.body && result.body.message) ? result.body.message : '주문자 정보 등록에 실패했습니다.');
						saveBtn.disabled = false;
						return;
					}
					...
					alert(result.body.message || '주문자 정보가 등록되었습니다.');
				})
				.catch(function() {
					alert('주문자 정보 등록 중 네트워크 오류가 발생했습니다.');
					saveBtn.disabled = false;
				});
		}
		...
		function deleteOrdererInfo(id) {
			if (!id) {
				alert('삭제할 주문자 정보가 올바르지 않습니다.');
				return;
			}

			if (!confirm('해당 주문자 정보를 삭제하시겠습니까?')) {
				return;
			}
			...
			fetch('/customer/api/orderer-infos/' + encodeURIComponent(id), {
				...
				.then(function(result) {
					if (!result.ok || !result.body || result.body.success === false) {
						alert((result.body && result.body.message) ? result.body.message : '주문자 정보 삭제에 실패했습니다.');
						return;
					}

					ordererItems = ordererItems.filter(function(x) {
						return String(x.id) !== String(id);
					});

					renderOrdererList();
					alert(result.body.message || '주문자 정보가 삭제되었습니다.');
				})
				.catch(function() {
					alert('주문자 정보 삭제 중 네트워크 오류가 발생했습니다.');
				});
		}
