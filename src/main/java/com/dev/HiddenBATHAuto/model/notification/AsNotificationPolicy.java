...
@Entity
@Table(
        name = "tb_as_notification_policy",
        uniqueConstraints = @UniqueConstraint(
                name = "uk_as_notification_policy_key",
                columnNames = {"source_area", "notification_action", "recipient_group"}
        ),
        indexes = @Index(
                name = "idx_as_notification_policy_lookup",
                columnList = "source_area,notification_action,recipient_group"
        )
)
@Getter
@NoArgsConstructor
public class AsNotificationPolicy {
    ...
    public static AsNotificationPolicy create(
        ...
        AsNotificationPolicy row = new AsNotificationPolicy();
        row.sourceArea = sourceArea;
        row.action = action;
        row.recipientGroup = recipientGroup;
        row.update(webEnabled, kakaoEnabled, importantEnabled, updatedByMemberId, updatedByUsername);
        return row;
    }

    public void update(boolean webEnabled, boolean kakaoEnabled, boolean importantEnabled,
                       Long updatedByMemberId, String updatedByUsername) {
