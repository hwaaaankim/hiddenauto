...
public interface ProductAttributeValueRepository
  ...
  boolean existsByGroupIdAndCustomerLabelIgnoreCaseAndIdNot(
      Long groupId, String customerLabel, Long id);

  boolean existsByGroupIdAndManagementLabelIgnoreCaseAndIdNot(
      Long groupId, String managementLabel, Long id);

  boolean existsByGroupIdAndProductionLabelIgnoreCaseAndIdNot(
      Long groupId, String productionLabel, Long id);

  @Query(
      "select coalesce(max(v.sortOrder), 0) from ProductAttributeValue v where v.group.id ="
          + " :groupId")
