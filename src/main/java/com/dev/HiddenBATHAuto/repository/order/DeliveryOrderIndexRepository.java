...
public interface DeliveryOrderIndexRepository extends JpaRepository<DeliveryOrderIndex, Long> {
    ...
    List<DeliveryOrderIndex> findByDeliveryHandlerAndDeliveryDateOrderByOrderIndex(
            Member handler,
            LocalDate date
    );

    int countByDeliveryHandlerAndDeliveryDate(
            Member handler,
            LocalDate date
    );
     ...
     * 기존 배송팀 리스트 조회
     * - 기존 메서드명 유지
     * - order / orderItem / productCategory / task / requestedBy / company / assignedDeliveryHandler fetch 추가
     * - 기존 화면에서 Lazy 로딩 문제를 줄이기 위한 안전 보강
     */
    @Query("""
        select distinct doi
        from DeliveryOrderIndex doi
    ...
    @Query("""
        ...
        where doi.deliveryHandler.id = :handlerId
          and doi.deliveryDate = :deliveryDate
    """)
    Integer findMaxIndexByHandlerAndDate(
            @Param("handlerId") Long handlerId,
            @Param("deliveryDate") LocalDate deliveryDate
    );
     ...
     * 업체별 정렬용
     * - 기존 메서드명 유지
     * - orderItem, productCategory, requestedBy.company fetch 추가
     */
    @Query("""
        select distinct doi
        from DeliveryOrderIndex doi
    ...
    /*
     * 기존 derived query를 명시 JPQL로 변경
     * - 메서드명은 그대로 유지
     * - 내부에서 필요한 order까지 fetch
     */
    @Query("""
        select doi
        from DeliveryOrderIndex doi
    ...
    @Query("""
        ...
        where doi.deliveryHandler.id = :deliveryHandlerId
          and doi.deliveryDate = :deliveryDate
    """)
    int findMaxOrderIndexByHandlerIdAndDeliveryDate(
            @Param("deliveryHandlerId") Long deliveryHandlerId,
            @Param("deliveryDate") LocalDate deliveryDate
    );
    ...
    /*
     * ============================================================
     * 신규 배송관리 페이지 /team/deliveryManager 전용
     * ============================================================
     *
     * 기준:
     * - 현재 로그인한 배송 담당자 기준
     * - Order.preferredDeliveryDate 범위 검색
     * - 상태는 서비스에서 CONFIRMED / PRODUCTION_DONE / DISPATCH_DONE / DELIVERY_DONE 전달
     * - 직배송 여부는 DeliveryManagerService에서 한 번 더 필터링
     */
    @Query("""
        select distinct doi
        from DeliveryOrderIndex doi
        ...
        order by
          ...
          o.id desc
    """)
    List<DeliveryOrderIndex> findDeliveryManagerBaseRows(
            @Param("handlerId") Long handlerId,
            @Param("statuses") List<OrderStatus> statuses,
