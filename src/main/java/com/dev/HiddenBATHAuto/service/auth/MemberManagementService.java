	...
	public List<ConflictDTO> saveMemberRegions(RegionBulkSaveRequest req) {
		...
		for (MemberRegion cand : candidates) {
			for (MemberRegion own : mine) {
				if (isOverlap(cand, own)) {
					conflicts.add(ConflictDTO.builder().conflictMemberId(me.getId())
							.conflictMemberName(me.getName() + " (본인)").conflictPath(toPath(own)).build());
				}
			}
		}
		...
		for (int i = 0; i < candidates.size(); i++) {
			for (int j = i + 1; j < candidates.size(); j++) {
				if (isOverlap(candidates.get(i), candidates.get(j))) {
					conflicts.add(ConflictDTO.builder().conflictMemberId(me.getId())
							.conflictMemberName(me.getName() + " (요청내 중복)").conflictPath(toPath(candidates.get(j)))
							.build());
				}
			}
		}
		...
		for (MemberRegion cand : candidates) {
			...
			for (MemberRegion o : othersSameTeam) {
				if (isOverlap(cand, o)) {
					conflicts.add(ConflictDTO.builder().conflictMemberId(o.getMember().getId())
							.conflictMemberName(o.getMember().getName()).conflictPath(toPath(o)).build());
				}
			}
		}
